        ...
        <div class="sub-gallery">
          ...
          >
            <li
              ...
              :class="{
                active: currentSlide - 1 === currentGroup * chunkSize + index,
              }"
            >
              <img :src="img.src" loading="eager" alt="sat" />
            </li>
          </ul>
        </div>
        <!-- list of images -->
...
const imagesList = computed(() =>
  props.galleryImages.map((img) => ({ src: img }))
);
...
.slider-wrapper .custom-prev {
  left: 5px;
}
.slider-wrapper .custom-next {
  right: 5px;
}
...
.slider-wrapper .slider-content-wrapper .form-select {
  border-radius: unset;
...
.slider-wrapper .slider-content-wrapper .text {
  color: #1e293b;
  font-family: "Inter", "Inter Fallback: Arial";
  font-size: 12px;
  font-weight: 600;
...
.slider-wrapper .sub-slider-images {
  display: flex;
  align-items: center;
  gap: 10px;
}
/* prev and next btn */
.slider-wrapper .sub-slider-images .previous-btn,
.slider-wrapper .sub-slider-images .next-btn {
  height: 110px;
  background: white;
  display: flex;
  align-items: center;
...
.slider-wrapper .sub-slider-images .previous-btn.disabled,
.slider-wrapper .sub-slider-images .next-btn.disabled {
  pointer-events: none;
  fill: #cbd5e1;
}
.slider-wrapper .sub-slider-images .previous-btn.active,
.slider-wrapper .sub-slider-images .next-btn.active {
  fill: #2a9fe8;
}
/* prev and next btn */

.slider-wrapper .sub-slider-images .sub-gallery {
  flex-grow: 1;
}
.slider-wrapper .sub-slider-images .sub-gallery ul {
  display: grid;
  grid-template-columns: repeat(8, 1fr);
  row-gap: 10px;
  column-gap: 6.03px;
}
.slider-wrapper .sub-slider-images .sub-gallery img {
  width: 49.69px;
  height: 49.69px;
  object-fit: cover;
  cursor: pointer;
}
.slider-wrapper .gallery-dynamic-count {
  position: absolute;
  bottom: 10px;
  left: 10px;
...
.slider-wrapper .sub-gallery ul li:hover::after {
  content: "";
  display: block;
  position: absolute;
  top: 0;
...
.slider-wrapper .sub-gallery ul li.active::after {
  content: "";
  display: block;
  position: absolute;
  top: 0;
...
.slider-wrapper .sub-gallery ul {
  list-style-type: none;
  padding: 0px;
  margin: 0px;
}
.slider-wrapper .sub-gallery ul li {
  position: relative;
}
</style>
